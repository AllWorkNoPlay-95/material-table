...
export default class MTableEditRow extends React.Component {
    ...
    renderColumns() {
        ...
        .map((columnDef, index) => {
            ...
            if (columnDef.editable === "onUpdate" && this.props.mode
                === "update") {
                allowEditing = true;
            }
            ...
            if (!columnDef.field || !allowEditing) {
                ...
                return (
                    <TableCell
                        ...
                        key={columnDef.tableData.id}
                        align={
                            ["numeric"].indexOf(columnDef.type) !== -1 ? "right"
                                : "left"
                        }
                        style={getCellStyle(columnDef, value)}
                    >
    ...
    renderActions() {
        ...
        const actions = [
            ...
            },
            {
                ...
                onClick: () => {
                    this.props.onEditingCanceled(this.props.mode,
                        this.props.data);
                },
            },
        ];
    ...
    handleKeyDown = (e) => {
        if (e.keyCode === 13 && e.target.type !== "textarea") {
            this.handleSave();
        } else if (e.keyCode === 13 && e.target.type === "textarea"
            && e.shiftKey) {
            this.handleSave();
        ...
        if (
            ...
            columns = [
                <TableCell
                    size={size}
                    padding={
                        this.props.options.actionsColumnIndex === 0 ? "none"
                            : undefined
                    }
                    key="key-edit-cell"
                    colSpan={colSpan}
                >
                    <Typography
                        variant="h6">{localization.deleteText}</Typography>
                </TableCell>,
            ];
        }
